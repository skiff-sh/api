...
func buildFileInfo(fd *descriptorpb.FileDescriptorProto) fileInfo {
	...
	// Top-level enums: path [5, i]
	for i, e := range fd.EnumType {
		enumPath := []int32{5, int32(i)}
		fi.Enums = append(fi.Enums, buildEnumInfo("", e, comments, enumPath))
	}

	// Walk all top-level messages (and their nested messages/enums) recursively.
...
// buildEnumInfo constructs enumInfo (with values) for a given enum.
func buildEnumInfo(
	prefix string,
	...
	name := e.GetName()
	goName := enumGoName(prefix, name)

	var values []enumValueInfo
	for _, v := range e.Value {
		// Const name: <TypeName>_<PROTO_VALUE_NAME>
		values = append(values, enumValueInfo{
			GoName: goName + "_" + v.GetName(),
			Number: v.GetNumber(),
		})
	}
	...
	return enumInfo{
		Name:    goName,
		Comment: comments[pathKey(enumPath)],
		Values:  values,
	}
}
...
func strconvI(v int) string {
	// tiny, allocation-free-ish int → string helper
	return strconv.Itoa(v)
}

// walkMessage flattens nested messages by emitting a type per message.
// Nested messages get a prefixed name, e.g. OuterInner.
...
func walkMessage(
	...
	// Nested enums: parentPath + [4, enumIndex]
	for i, e := range m.EnumType {
		enumPath := append(append([]int32{}, msgPath...), 4, int32(i))
		fi.Enums = append(fi.Enums, buildEnumInfo(prefix+name+"_", e, comments, enumPath))
	}
...
func goTypeForMapValue(
	...
	case descriptorpb.FieldDescriptorProto_TYPE_ENUM:
		// Map values of enum type are represented as their enum alias type.
		return goEnumNameFromTypeName(fd, f.GetTypeName())
	default:
		return "interface{}"
	}
}
...
func goTypeForField(
	...
	case descriptorpb.FieldDescriptorProto_TYPE_ENUM:
		// Enum fields are represented as their alias type.
		base = goEnumNameFromTypeName(fd, f.GetTypeName())
	default:
		// For anything else you haven't handled yet.
		base = "interface{}"
	}
	...
	// Handle proto3 optional: map to pointer type for primitives and enums.
...
// to the generated Go struct name ("OuterInner" with current prefix handling).
func goMessageNameFromTypeName(
	...
	// Our walkMessage uses "OuterInner" naming for nested.
	return strings.Join(segments, "")
}

// goEnumNameFromTypeName maps a protobuf enum type name (".pkg.Outer.Inner.Enum")
// to the generated Go enum alias name ("OuterInnerEnum").
func goEnumNameFromTypeName(
	fd *descriptorpb.FileDescriptorProto,
	typeName string,
) string {
	if !strings.HasPrefix(typeName, ".") {
	...
	var enumPath string
	if pkg != "" && strings.HasPrefix(typeName, pkg+".") {
		enumPath = strings.TrimPrefix(typeName, pkg+".")
	} else {
		enumPath = typeName
	}

	segments := strings.Split(enumPath, ".") // ["Outer", "Inner", "Enum"]
	for i, s := range segments {
		segments[i] = toExported(s)
	}
	// Match enumGoName / buildEnumInfo naming: "OuterInnerEnum".
	return strings.Join(segments, "")
}

// enumGoName computes the Go name for an enum given a prefix (for nesting)
// and the enum's own name, following the same rules as nested messages.
//
// Example: prefix "Outer_" and name "InnerStatus" -> "OuterInnerStatus".
func enumGoName(prefix, name string) string {
	return toExported(prefix + name)
}
